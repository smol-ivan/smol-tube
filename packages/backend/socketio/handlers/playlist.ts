...
async function fetchVideoMeta(videoId: string): Promise<VideoMeta> {
    ...
    if (!apiKey) {
        console.warn(
            "[playlist] YOUTUBE_API_KEY no configurada — usando fallback de metadatos",
        );
        return fallback;
    }
    ...
    try {
        ...
        if (!res.ok) {
            console.warn(
                `[playlist] YouTube Data API respondió ${res.status} para videoId=${videoId}`,
            );
            return fallback;
        }
        ...
        if (!item) {
            console.warn(
                `[playlist] YouTube Data API: no se encontró el video ${videoId}`,
            );
            return fallback;
        }
        return {
...
export function handlePlaylistEvents(io: Server, socket: Socket) {
    ...
    socket.on(
        ...
        async (payload: PlaylistRemovePayload, ack?: AckFn<{ room: Room }>) => {
            ...
            const targetItem = context.room.playlist.find(
    ...
    socket.on(
        ...
        async (payload: SkipVotePayload, ack?: AckFn<{ room: Room }>) => {
            ...
            const roomSize =
                io.sockets.adapter.rooms.get(context.room.roomId)?.size || 1;
