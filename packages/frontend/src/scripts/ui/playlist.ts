...
export function renderPlaylist(): void {
    ...
    if (state.history.length === 0) {
        if (elements.playlistHistory)
            elements.playlistHistory.classList.add("hidden");
        if (elements.playlistHistory)
            elements.playlistHistory.classList.remove("flex");
    } else {
        if (elements.playlistHistory)
            elements.playlistHistory.classList.remove("hidden");
        if (elements.playlistHistory)
            elements.playlistHistory.classList.add("flex");
        if (elements.playlistHistoryItems) {
            elements.playlistHistoryItems.innerHTML = "";
            for (const item of state.history) {
                const row = document.createElement("div");
                row.className =
                    "flex items-center gap-2 px-2 py-1 border-b border-outline-variant/50 bg-surface-container/30 opacity-75";
                ...
                title.target = "_blank";
                title.className =
                    "font-label-sm text-label-sm text-on-surface-variant truncate hover:underline hover:text-secondary";
    ...
    if (state.playlist.length === 0) {
        ...
        for (const [index, item] of state.playlist.entries()) {
            const isPlaying =
                state.playback.videoId === item.videoId && index === 0;

            const row = document.createElement("div");
            row.className = `flex items-center gap-2 p-2 border-b border-outline-variant cursor-pointer hover:bg-surface-container-highest group ${isPlaying ? "bg-surface-container-highest border-l-4 border-l-primary" : "bg-surface-container-high border-l-4 border-l-secondary"}`;
                ...
                if (e.dataTransfer) {
                    ...
                    if (fromIndexStr) {
                        ...
                        if (fromIndex !== toIndex) {
                            state.socket.emit("playlistReorder", {
                                fromIndex,
                                toIndex,
                            });
                        }
            ...
            if (
                state.playback.leaderUserId === state.currentUserId ||
                state.currentRole === "admin"
            ) {
                const actions = document.createElement("div");
                actions.className =
                    "hidden group-hover:flex items-center gap-1 text-on-surface-variant";
                ...
                if (!isPlaying) {
                    ...
                    playBtn.addEventListener("click", (e) => {
                        e.stopPropagation();
                        state.socket.emit("playlistPlay", {
                            itemId: item.itemId,
                        });
                    });
                ...
                if (index < state.playlist.length - 1 && index > 0) {
                    ...
                    bottomBtn.addEventListener("click", (e) => {
                        ...
                        state.socket.emit("playlistReorder", {
                            fromIndex: index,
                            toIndex: state.playlist.length - 1,
                        });
                    });
                ...
                if (index > 0) {
                    ...
                    delBtn.addEventListener("click", (e) => {
                        e.stopPropagation();
                        state.socket.emit("playlistRemove", {
                            itemId: item.itemId,
                        });
                    });
            ...
            duration.className =
                "font-label-sm text-label-sm text-on-surface-variant whitespace-nowrap group-hover:hidden";
    ...
    // Update footer
    const totalSeconds = state.playlist.reduce(
        (acc, i) => acc + i.durationSeconds,
        0,
    );
